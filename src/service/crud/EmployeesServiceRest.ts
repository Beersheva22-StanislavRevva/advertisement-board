...
class Cache {
    cacheString: string = '';
    set(employees: Adv[]): void {
        this.cacheString = JSON.stringify(employees);
    }
    reset() {
        this.cacheString = ''
    }
    isEqual(employees: Adv[]): boolean {
...
async function fetchRequest(url: string, options: RequestInit, empl?: Adv): Promise<Response> {
...
async function fetchAllEmployees(url: string):Promise< Adv[]|string> {
...
export default class EmployeesServiceRest implements EmployeesService {
    ...
    async updateEmployee(empl: Adv): Promise<Adv> {
    ...
    private sibscriberNext(url: string, subscriber: Subscriber<Adv[] | string>): void {
        ...
        fetchAllEmployees(url).then(employees => {
            if (this.cache.isEmpty() || !this.cache.isEqual(employees as Adv[])) {
                this.cache.set(employees as Adv[]);
    ...
    async addEmployee(empl: Adv): Promise<Adv> {
